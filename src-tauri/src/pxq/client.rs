...
const UA: &str = "Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1";
const HOST: &str = "m.piaoxingqiu.com";
pub const API_VER: &str = "4.1.2-20240305183007";
pub const API_SRC: &str = "WEB";
const TERMINAL_SRC: &str = "WEB";
...
fn box_str(td: &str) -> String {
    ...
    td_chars[0] = random_str(1).chars().next().unwrap();
    td_chars.insert(4, random_str(1).chars().next().unwrap());
    td_chars.insert(15, random_str(1).chars().next().unwrap());
    td_chars.insert(td_chars.len() - 1, random_str(1).chars().next().unwrap());
